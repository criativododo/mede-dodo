...
# SPEC-006 §2.2: exportação compacta em formato longo (tidy — uma linha por
# métrica), pensada para consumo por planilha ou por ferramentas de IA que
# preferem tabela plana a JSON aninhado. Não recalcula nada: só serializa os
# mesmos campos já expostos por generate_html_report/generate_pdf_report.
_CSV_HEADER = ["secao", "campo", "valor", "procedencia"]


def _csv_rows_formatos(metrics_map):
    formats = (metrics_map.get("format_metrics") or {}).get("formats") or {}
    rows = []
    for formato, dados in formats.items():
        secao = f"formato_{formato}"
        if dados.get("status") == "ok":
            rows.append([secao, "post_count", dados.get("post_count", 0), "observado"])
            rows.append([secao, "average_likes", _fmt_float(dados.get("average_likes")), "derivado"])
            rows.append([secao, "average_comments", _fmt_float(dados.get("average_comments")), "derivado"])
            rows.append([secao, "engagement_rate_pct", _fmt_float(dados.get("engagement_rate")), "derivado"])
        else:
            rows.append([secao, "status", "indisponivel", "—"])
    return rows


def _csv_rows_autenticidade(analysis, metrics_map):
    rows = []
    autenticidade = metrics_map.get("audience_authenticity_signal") or {}
    if autenticidade.get("status") == "ok":
        rows.append(["autenticidade", "base_real_pct", _fmt_float(autenticidade.get("value")), "estimado"])
        rows.append(["autenticidade", "confidence", autenticidade.get("confidence") or "N/D", "estimado"])
    else:
        fallback = analysis.get("fake_followers_estimate") or {}
        if fallback.get("value") is not None:
            rows.append(
                ["autenticidade", "risco_pct_legado", _fmt_float(fallback.get("value")), "estimado — legado"]
            )
        else:
            rows.append(["autenticidade", "status", "indisponivel", "—"])
    return rows


def _csv_rows_demografia(metrics_map):
    rows = []
    gender_metric = metrics_map.get("gender_distribution") or {}
    if gender_metric.get("genero_validado_n"):
        rows.append(["demografia", "feminino_pct", _fmt_float(gender_metric.get("feminino_pct")), "derivado"])
        rows.append(["demografia", "masculino_pct", _fmt_float(gender_metric.get("masculino_pct")), "derivado"])
        rows.append(
            [
                "demografia",
                "coverage_gender_pct",
                _fmt_float((gender_metric.get("coverage_gender") or 0.0) * 100),
                "derivado",
            ]
        )
    else:
        rows.append(["demografia", "status", "indisponivel", "—"])
    return rows


def _csv_rows_parcerias(analysis):
    rows = []
    publis = analysis.get("publis") or []
    for idx, item in enumerate(publis, start=1):
        secao = "parceria"
        # BRAND_MENTION_PATTERN (src/filters.py) captura o handle sem "@" —
        # mesma normalização de exibição de app.py._render_parceria_mini_card
        # e de metrics.extract_brand_mentions (f"@{handle}").
        marcas_normalizadas = [f"@{marca.lstrip('@')}" for marca in (item.get("marcas") or [])]
        marcas = ", ".join(marcas_normalizadas) or "sem marca identificada"
        rows.append([secao, f"post_{idx}_marcas", marcas, "observado"])
        rows.append([secao, f"post_{idx}_link", item.get("link") or "indisponivel", "observado"])
    return rows


def generate_csv_report(analysis: dict) -> bytes:
    """SPEC-006 §2.2: uma linha por métrica (secao, campo, valor, procedencia).
    Não recalcula fórmulas nem altera src/metrics.py — leitura pura do mesmo
    dicionário `analysis` já usado por generate_html_report/generate_pdf_report."""
    audit_report = analysis.get("audit_report") or {}
    metrics_map = audit_report.get("metrics") or {}

    rows = [
        ["identidade", "username", analysis.get("username", ""), "observado"],
        ["identidade", "window_days", analysis.get("window_days", ""), "observado"],
        ["identidade", "followers_count", analysis.get("followers_count", 0), "observado"],
        [
            "kpis",
            "engagement_rate_pct",
            _fmt_float((analysis.get("engagement_rate") or 0.0) * 100),
            "derivado",
        ],
        ["kpis", "average_likes", _fmt_float(analysis.get("average_likes")), "observado"],
        ["kpis", "average_comments", _fmt_float(analysis.get("average_comments")), "observado"],
        ["kpis", "score_dodo", _fmt_float(analysis.get("score_dodo")), "derivado — legado"],
    ]
    rows.extend(_csv_rows_formatos(metrics_map))
    rows.extend(_csv_rows_autenticidade(analysis, metrics_map))
    rows.extend(_csv_rows_demografia(metrics_map))
    rows.extend(_csv_rows_parcerias(analysis))
