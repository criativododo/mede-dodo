...
def summarize_gender_distribution_valid(comments, names_db=DEFAULT_NAMES_DB):
    """Composição de gênero normalizada pela amostra válida e reconhecida
    (SPEC-005 §4.1, ISSUE-0010): o percentual de cada gênero é dividido por
    `feminino_validado + masculino_validado`, nunca pelo total bruto de
    comentários — evita o erro de denominador (ex.: 10 nomes femininos em 160
    comentários brutos virando "6.2% feminino" em vez da composição real da
    amostra reconhecida). Contrato aditivo: não substitui
    `summarize_gender_distribution` (mantida intocada, ainda consumida por
    `metrics.calc_gender_distribution_metric` para os campos legados
    "percentuais"/"cobertura"). Lista vazia -> zeros, nunca lança exceção."""
    total = len(comments)
    counts = {"feminino": 0, "masculino": 0, "indeterminado": 0}
    ...
    feminino_validado = counts["feminino"]
    masculino_validado = counts["masculino"]
    genero_validado_n = feminino_validado + masculino_validado

    return {
        "feminino_validado": feminino_validado,
        "masculino_validado": masculino_validado,
        "genero_validado_n": genero_validado_n,
        "genero_unknown_n": counts["indeterminado"],
        "feminino_pct": (feminino_validado / genero_validado_n * 100) if genero_validado_n else 0.0,
        "masculino_pct": (masculino_validado / genero_validado_n * 100) if genero_validado_n else 0.0,
        "coverage_gender": (genero_validado_n / total) if total else 0.0,
    }
