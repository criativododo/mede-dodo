...
def instaloader_fetch_fn(username, cookies=None, rate_controller=None, on_progress=None):
    ...
    # SPRINT-003 (investigação de performance): o default do Instaloader
    # (3 tentativas) trata qualquer HTTP 200 com corpo {"status":"fail"} como
    # erro de conexão comum e reenvia a MESMA requisição — mas o "fail"
    # observado no endpoint de comentários (_DELETED_SCHEMA_SIGNATURE tem bug
    # irmão neste mesmo padrão) é reproduzido de forma 100% consistente em
    # dezenas de posts/perfis diferentes ao longo de vários dias (ver
    # PROGRESS.md), não um blip transitório de rede — não há motivo para uma
    # 3ª tentativa idêntica. Reduzir para 2 tentativas (nunca 1, para manter
    # um retry de segurança contra blips reais na resolução de perfil/
    # paginação de posts) só CORTA requisições — nunca adiciona, nunca reduz
    # o jitter/throttle real (DUMMY.md #3), e não muda em nada a detecção de
    # 429/checkpoint (que já interrompe na 1ª ocorrência via
    # TooManyRequestsException, independente deste número). É puramente
    # menos desperdício contra um bug já comprovado não-transitório.
