...
## 4. Anexo: Estado de Implementação (atualizado 2026-08-12 — reparo dos gargalos de coleta do Instagram, ISSUE-0001)
...
| RF-03 Coleta local | Feito | ISSUE-0001 / ISSUE-0008 | `instaloader_fetch_fn` (via lib `instaloader`, sessão local por arquivo de cookies em `INSTAGRAM_SESSION_FILE`) implementado e integrado ao pipeline de `app.py`; cache/throttling/orquestração/fallback também prontos. Desde a ISSUE-0008 também busca comentários reais (`post.get_comments()`) e a data real de publicação — antes só coletava metadados agregados (likes/contagem de comentários). Reparado em 2026-08-12 (ver ISSUE-0001.md): `load_any_available_session` autodetecta sessão salva em `~/.config/instaloader/session-*` (sem depender de `INSTAGRAM_SESSION_FILE`) e corrige um bug de identidade trocada (username do perfil analisado sendo usado como dono da sessão), que também explica o Erro HTTP 400 em perfis business/creator (requisição anônima cai no endpoint público instável, sessão autenticada usa GraphQL); sidebar do Streamlit agora mostra o status da sessão. Validação com uma chamada real ao Instagram (login/cookies de verdade contra o serviço live) ainda não foi feita neste ambiente — evitada deliberadamente nesta sessão automatizada por ser uma ação de rede não supervisionada ao vivo contra a conta real do usuário |
