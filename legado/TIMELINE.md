...
## 2026-08-15 (Sprint 004 — ISSUE-0010/SPEC-005: correção de demografia, formatos
comparáveis e remoção do Score DODÔ do topo)
- Executado em worktree próprio (`worktree-mede-dodo-sprint003`), fatiamento A (fixtures) →
  B (`src/`) → C (`app.py`) → D (validação/exportação).
- `src/demographics.py`: `summarize_gender_distribution_valid` (nova, aditiva) corrige o
  denominador de gênero para `feminino_validado + masculino_validado` (amostra reconhecida),
  em vez do total bruto de comentários.
- `src/metrics.py`: `calculate_format_metrics` (Reels/Carrossel/Estático com n, likes/
  comentários médios e ER próprios, nunca zero silencioso quando indisponível);
  `calibrate_authenticity_composition` separa `base_real_pct`/`suspeito_pct`/
  `desconhecido_pct` com piso proporcional ao tamanho da amostra; aliases de nomenclatura
  SPEC-005 em `calc_creator_response_rate_metric`; `build_audit_report` de 12 para 13 campos
  (`+format_metrics`).
- `app.py`: Score DODÔ/parecer comercial removidos do topo, movidos para "Detalhes da
  auditoria" (rotulados como legado/derivado); `_render_format_performance` reescrita com 3
  cards comparáveis; `_render_audience_profile` com barras de gênero pela amostra válida e
  `coverage_gender` explícita.
- Validação visual manual via Playwright contra `streamlit run app.py` (Modo Demonstração,
  desktop 1440×1000 e viewport estreito 390×844), console sem erros.
- Suíte de testes: 305 → 337 (32 testes novos), 0 regressões. `ISSUE-0010` concluída
