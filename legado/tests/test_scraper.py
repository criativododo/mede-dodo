...
class FakeInstaloader:
    def __init__(self, **kwargs):
        ...
        self.max_connection_attempts = kwargs.get("max_connection_attempts")
...
def test_instaloader_fetch_fn_reduces_max_connection_attempts_to_avoid_wasted_retries(monkeypatch):
    """SPRINT-003 (investigação de performance): o "fail" no endpoint de
    comentários é reproduzido de forma 100% consistente (não é blip
    transitório) — a 3ª tentativa idêntica do default do Instaloader é
    desperdício puro. 2 (não 1) preserva um retry de segurança contra blips
    reais em outras chamadas (resolução de perfil/paginação de posts)."""
    created_loaders = []
    ...
    scraper.instaloader_fetch_fn("perfil_fake", cookies=None)

    assert len(created_loaders) == 1
    assert created_loaders[0].max_connection_attempts == 2
